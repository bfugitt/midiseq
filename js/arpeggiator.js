...
export function calculateArpNote(notes, index, mode, octaves, chordSequence) {
    ...
    if (chordSequence === 'held_notes' || !intervals) {
        ...
        let effectiveIndex;
        switch (mode) {
            case 'up': effectiveIndex = index % totalArpNotes; break;
            case 'down': effectiveIndex = totalArpNotes - 1 - (index % totalArpNotes); break;
            case 'updown':
                ...
                break;
            case 'random': effectiveIndex = Math.floor(Math.random() * totalArpNotes); break;
            default: effectiveIndex = 0;
        }
...
function runArpStep() {
    ...
    // Kill all notes to ensure monophonic arp
    ...
    if (noteToPlay !== null) {
        ...
        setTimeout(() => sendMidiNoteOff(noteToPlay), intervalTimeMs * 0.9);
    }
